	...
	public static void main(String[] args) {
		...
		try {
			...
			if ((args.length != 1) && (args.length != 2)) {
				System.out.println("Usage GetFriends <id or screenName>");
				System.exit(1);
			}
			...
			if (args.length == 1) {
				do {
					cursor = new_cursor;
					ids = twitter.getFriendsIDs(nom_user, cursor);
				...
				System.out
						.println("Le dernier cursor est "
								+ cursor
								+ " vous devriez relancer le programme avec GetFriends "
								+ nom_user + " " + cursor);
//				System.exit(0);
			} else {
			...
			while (cursor != 0) {
				...
				for (i = 0; i < tab_ids.length; i++) {
					base.insertLink(u.getId(), tab_ids[i]);
				}
